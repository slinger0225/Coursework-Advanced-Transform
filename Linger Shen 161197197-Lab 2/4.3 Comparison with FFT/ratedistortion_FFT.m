function ratedistortion_FFT(maxthreshold)
...
for threshold=0:step:maxthreshold
   ...
   ratevec = [ratevec comp_ratio_FFT(threshold)];
   % Add the distortion value to the "distortion" curve
   distvec = [distvec distortion_FFT(threshold)];
end
...
ylabel('Distortion');
title('rate-distortion curve for FFT (1D),threshold=1.6392')